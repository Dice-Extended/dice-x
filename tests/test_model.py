...
class TestBaseModelLoader:
    ...
    def test_tf(self):
        ...
        assert issubclass(type(m), dice_ml_x.model_interfaces.base_model.BaseModel)
        assert isinstance(m, dice_ml_x.model_interfaces.keras_tensorflow_model.KerasTensorFlowModel)
    ...
    def test_sklearn(self):
        ...
        backend = 'sklearn'
        m = self._get_model(backend)
        assert isinstance(m, dice_ml_x.model_interfaces.base_model.BaseModel)
...
class TestModelUserValidations:
    ...
    def test_model_user_validation_model_type(self):
        ...
        assert dice_ml_x.Model(model=trained_model, backend='sklearn', model_type='classifier') is not None
        assert dice_ml_x.Model(model=trained_model, backend='sklearn', model_type='regressor') is not None

        with pytest.raises(UserConfigValidationException):
            dice_ml_x.Model(model=trained_model, backend='sklearn', model_type='random')

    def test_model_user_validation_no_valid_model(self):
        with pytest.raises(
                ValueError,
                match="should provide either a trained model or the path to a model"):
            dice_ml_x.Model(backend='sklearn')
