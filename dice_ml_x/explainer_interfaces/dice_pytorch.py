...
class DicePyTorch(ExplainerBase):
    ...
    def generate_perturbations(self, method: str, max_iter=100,
                               tol=1e-3, gamma=1e-2, **kwargs):
        """
        Generates perturbed counterfactuals for counterfactuals generated by this class.
        ...
        Args:
            ...
            gamma (float): The weight of the penalty term that will be added to the loss function
                measuring the distance between predictions that are made with the original
                counterfactual instance and the perturbed counterfactual instance.
        ...
        perturbed_cfs = []
        cfs = np.array([self.cfs[i].detach().cpu().numpy() for i in range(len(self.cfs))])
        ...
        for idx, c_i in cfs_df.iterrows():
            ...
            c_i_numerical = self.cfs[idx].clone().detach().float()

            c_i_prime_df = perturbation_instance.generate(c_i=c_i_df)
            c_i_prime_numerical = torch.tensor(
                                               self.model.transformer.transform(c_i_prime_df).values,
                                               dtype=torch.float32,
                                               requires_grad=True
                                               )
            
            perturbation_optimizer = torch.optim.Adam([c_i_prime_numerical], lr=1e-3)
            
            prev_loss = np.inf
            for it in range(max_iter):
                with torch.no_grad():
                    self.model.model.eval()
                    pred_i = self.model.model(c_i_numerical)
                    pred_i_prime = self.model.model(c_i_prime_numerical)
                class_loss = torch.mean((pred_i - pred_i_prime) ** 2)
                distance = torch.norm(c_i_prime_numerical - c_i_numerical, p=2)
            ...
            optimized_c_i_prime = pd.DataFrame(
                self.model.transformer.inverse_transform(
                    self.data_interface.get_decoded_data(
                        c_i_prime_numerical.detach().numpy()
                    )
                ),
    ...
    def _preprocess_for_robustness(self, cfs: pd.DataFrame, perturbed_cfs: pd.DataFrame) -> tuple:
        ...
        def preprocess_continuous_features(data: pd.DataFrame):
            ...
            for col in continuous_cols:
                ...
                binned = np.digitize(data[col], bins, right=False) - 1
    ...
    def compute_robustness_loss(self, perturbed_cfs_df: pd.DataFrame) -> float:
        ...
        """
        cfs = np.array([self.cfs[i].detach().numpy() for i in range(len(self.cfs))])
        cfs_df = self.model.transformer.inverse_transform(
                self.data_interface.get_decoded_data(cfs))
        ...
        return torch.tensor(np.mean(sorensen_dice_coefficient), dtype=torch.float32, requires_grad=True)
    ...
    def _preprocess_for_robustness(self, cfs: pd.DataFrame, perturbed_cfs: pd.DataFrame) -> tuple:
        ...
        """
        def preprocess_continuous_features(data: pd.DataFrame, bins_dict: dict, continuous_cols: list) -> torch.Tensor:
            one_hot_tensors = []
            for col in continuous_cols:
                bins = bins_dict[col]
                
                col_values = torch.tensor(data[col].values, dtype=torch.float32)
                binned_indices = torch.bucketize(col_values, bins, right=False) - 1
                binned_indices = torch.clamp(binned_indices, min=0, max=len(bins) - 2)

                num_bins = len(bins) - 1
                one_hot = torch.nn.functional.one_hot(binned_indices, num_classes=num_bins)
                one_hot_tensors.append(one_hot.float())
                
            return torch.cat(one_hot_tensors, dim=1)
        
        def preprocess_categorical_features(data: pd.DataFrame, categorical_cols: list, category_mappings: dict):
            one_hot_tensors = []
            for col in categorical_cols:
                indices = torch.tensor(data[col].map(category_mappings[col]).values, dtype=torch.long)
                num_classes = len(category_mappings[col])
        ...
        combined_data = pd.concat([cfs, perturbed_cfs], ignore_index=True)
        category_mappings = {col: {val: idx for idx, val in enumerate(combined_data[col].unique())} for col in categorical_cols}

        cfs_categorical = preprocess_categorical_features(cfs, categorical_cols, category_mappings)
        perturbed_cfs_categorical = preprocess_categorical_features(perturbed_cfs, categorical_cols, category_mappings)
        ...
        for col in continuous_cols:
            col_min, col_max = ranges[col]
            bins_dict[col] = torch.linspace(col_min, col_max, steps=num_bins + 1)

        cfs_continuous = preprocess_continuous_features(cfs, bins_dict, continuous_cols)
        perturbed_cfs_continuous = preprocess_continuous_features(perturbed_cfs, bins_dict, continuous_cols)

        cfs_processed = torch.cat([cfs_continuous, cfs_categorical], dim=1)
        perturbed_cfs_processed = torch.cat([perturbed_cfs_continuous, perturbed_cfs_categorical], dim=1)

        return cfs_processed, perturbed_cfs_processed
    

    def compute_robustness_loss(self, perturbed_cfs_df: pd.DataFrame) -> torch.Tensor:
        """
        Computes the robustness loss.
        ...
        Returns:
            float: Robustness loss in scalar.
        """ 
        cfs = torch.tensor([self.cfs[i].detach().numpy() for i in range(len(self.cfs))], dtype=torch.float32, requires_grad=True)
        cfs_df = self.model.transformer.inverse_transform(
            self.data_interface.get_decoded_data(cfs.detach().numpy())   
        )

        cfs_processed, perturbed_cfs_processed = self._preprocess_for_robustness(cfs_df, perturbed_cfs_df)

        intersection = torch.sum(torch.min(cfs_processed, perturbed_cfs_processed), dim=1)
        intersection.requires_grad = True
        union = torch.sum(cfs_processed, dim=1) + torch.sum(perturbed_cfs_processed, dim=1)
        union.requires_grad = True
    ...
    def compute_loss(self, method: str, **kwargs):
        ...
        perturbed_cfs_df = self.generate_perturbations(method, **kwargs)
        self.robustness_loss = self.compute_robustness_loss(perturbed_cfs_df=perturbed_cfs_df)
